...
const ARCHETYPES: BotArchetype[] = [
  { name: "Blitz-Bernd", emoji: "⚡", smarts: 0.92, pace: 0.55 },
  { name: "Grübel-Gabi", emoji: "🤔", smarts: 1.12, pace: 1.35 },
  { name: "Klugscheißer-Klaus", emoji: "🤓", smarts: 1.18, pace: 1.0 },
  { name: "Chaos-Cem", emoji: "🌀", smarts: 0.72, pace: 0.75 },
  { name: "Oma Ottilie", emoji: "🧶", smarts: 1.0, pace: 1.45 },
  { name: "Turbo-Tina", emoji: "🏎️", smarts: 0.84, pace: 0.5 },
  { name: "Lexi Lexikon", emoji: "📚", smarts: 1.25, pace: 1.15 },
  { name: "Faul-Fred", emoji: "😴", smarts: 0.6, pace: 1.6 },
  { name: "Pixel-Pia", emoji: "👾", smarts: 0.95, pace: 0.85 },
  { name: "Doktor Dodo", emoji: "🦤", smarts: 0.8, pace: 1.1 },
];
...
export function makeBots(count: number, difficulty: GameSettings["botDifficulty"]): Player[] {
  const base = DIFFICULTY[difficulty];
  const pool = [...ARCHETYPES].sort(() => Math.random() - 0.5).slice(0, Math.max(0, count));
  return pool.map((a, i) => ({
    id: `bot-${i}-${a.name.toLowerCase().replace(/[^a-z]/g, "")}`,
    name: a.name,
    emoji: a.emoji,
    kind: "bot" as const,
...
export type BotPlan = {
  ...
  doneAtMs: number;
  /** Relative ms when the bot slams the Stopp button, or null if it never will. */
...
export function planBotRound(
  ...
  for (const category of categories) {
    const known = wordsFor(category.bank, letter).filter((w) => !used.has(w.toLowerCase()));
    ...
    const candidates = isKnown ? known : genericWords(letter);
  ...
  // Only a bot that got everything - and is feeling cocky - calls Stopp.
  const stopAtMs =
    settings.allowStop && filledAll && Math.random() < 0.55 + skill * 0.35
      ? Math.min(doneAtMs + 300 + Math.random() * 900, limitMs - 500)
      : null;

  return { playerId: bot.id, fills, doneAtMs: Math.round(doneAtMs), stopAtMs: stopAtMs ? Math.round(stopAtMs) : null };
}
